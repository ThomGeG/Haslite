...
/**
 * Conduct syntax analysis on the Haslite program in the file given as the
...
object Main extends SyntaxAnalysis with ParsingREPL[Program] {
    ...
    def compileAndRun (program : Program) {
        ...
        if (messages.length > 0)
            report (messages) //If there were problems with the input, report them to the user and don't continue.
        else {
